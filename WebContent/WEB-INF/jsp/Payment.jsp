	...
	var TableInit = function() {
		...
		oTableInit.Init = function() {
			$('#tb_patient_message').bootstrapTable({
				url: '/hospital/selectAll_Payment_Patient_message', //请求后台的URL（*）
				method: 'get', //请求方式（*）
		...
		oInit.Init = function() {
			...
			$(document).on('click', "#btn_show",function(){									
				...
				if (arrselectedData.length <= 0) {
                	...
                	turnnew();
					function turnnew(){
						page("Payment2?patient_id="+row.patient.patient_id+"&patient_name="+row.patient.patient_name
								+"&deparment_name="+row.deparment.deparment_name+"&room_name="+row.room.room_name
								+"&bed_name="+row.bed.bed_name,row.patient.patient_name+"的预交金详情");
