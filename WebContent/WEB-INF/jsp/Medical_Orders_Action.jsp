	...
	var TableInit = function() {
		...
		oTableInit.Init = function() {
			$('#tb_patient_message').bootstrapTable({
				url: '/hospital/select_today_Action_Medical_Orders', //请求后台的URL（*）
				method: 'get', //请求方式（*）
				...
				columns: [{
					...
					field: 'patient_message.ruyuan_time',
					title: '入院日期',
					formatter: function (value, row, index) {
		...
		oTableInit.queryParams = function(params) {
			var param = { //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
				...
				deparment_id : "${USER.deparment_id}",
		...
		oInit.Init = function() {
			...
			$(document).on('click', "#btn_action",function(){
				...
				if (arrselectedData.length <= 0) {
                	...
                	turnnew();
					function turnnew(){
						page("Medical_Orders_Action2?patient_id="+row.patient.patient_id+"&patient_name="+row.patient.patient_name
								+"&deparment_name="+row.deparment.deparment_name+"&room_name="+row.room.room_name
								+"&bed_name="+row.bed.bed_name+"&medical_orders_id="+row.medical_orders_action_id,"医嘱校对执行");
