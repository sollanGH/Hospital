	...
	var TableInit = function() {
		...
		oTableInit.Init = function() {
			$('#tb_patient_message').bootstrapTable({
				url: '/hospital/select_Action_Medical_Orders', //请求后台的URL（*）
				method: 'get', //请求方式（*）
		...
		oInit.Init = function() {
			...
			$(document).on('click', "#btn_action",function(){
				...
				if (arrselectedData.length <= 0) {
                	...
                	turnnew();
					function turnnew(){
						page("Medical_Orders_Check2?patient_id="+row.patient.patient_id+"&patient_name="+row.patient.patient_name
								+"&deparment_name="+row.deparment.deparment_name+"&room_name="+row.room.room_name
								+"&bed_name="+row.bed.bed_name+"&medical_orders_id="+row.medical_orders_id,"校对列表");
