	...
	<div class="panel-body" style="padding-bottom:0px;">
		...
		<div id="toolbar" class="btn-group">					
	   		<button id="btn_add" type="button" class="btn btn-white">
              	<span class="glyphicon glyphicon-pencil" ></span>录入检验结果
	          	</button>
	...
	var TableInit = function() {
		...
		oTableInit.Init = function() {
			$('#tb_patient_message').bootstrapTable({
				url: '/hospital/select_action_Inspection', //请求后台的URL（*）
				method: 'get', //请求方式（*）
	...
	var ButtonInit = function() {
		...
		oInit.Init = function() {
			...
			$(document).on('click', "#btn_add",function(){									
				...
				if (arrselectedData.length <= 0) {
                ...
                }/* else if(row.inspection_result == 1){
                	swal({
	                	   title:"已填写",
	                	   text:"",
	                	   type:"warning"
                	...
                	for (var i = 0; i < arrselectedData.length; i++) {
                		select_id += arrselectedData[i]['inspection_id'] + ",";
                    }
                	//去掉最后的逗号
			        ...
			        $.ajax({
						url: "/hospital/action_Inspection",
						type:"post",
						data:{
							select_id : select_id,
							Inspection_id: select_id,							
						},
						 success: function (data) {
							/*  swal({
			                	   title:"成功",
			                ...
			                //跳转至执行页面
			                	turnnew();
								function turnnew(){
									page("Inspection_action2?patient_id="+row.patient.patient_id+"&inspection_id="+row.inspection_id
