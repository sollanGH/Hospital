	 ...
	 <body>
		...
		<div id="toolbar" class="btn-group">
			<button id="btn_check" type="button" class="btn btn-white" >
	        	<span class="glyphicon glyphicon-pencil"></span>药品核对退药
	    	</button>
	   		<button id="btn_check_cancel" type="button" class="btn btn-white">
              	<span class="glyphicon glyphicon-remove" ></span>取消核对退药
          	</button>
          	<button id="btn_action" type="button" class="btn btn-white">
              	<span class="glyphicon glyphicon-pencil" ></span>药品退药
          	</button>
    	</div>
		...
		oTableInit.Init = function() {
			$('#tb_patient_message').bootstrapTable({
				url: '/hospital/select_cancelRequisition_List', //请求后台的URL（*）
				method: 'get', //请求方式（*）
				...
				columns:
					[
	                 ...
	                 [
	                    {
	                         field: 'check_return_requisition_time',
	                         title: '核对时间',
	                        valign:"middle",
	                         align:"center",
	                      ...
	                      {
	                         field: 'action_return_requisition_time',
	                        title: '退药时间',
	                         valign:"middle",
	                         align:"center",
	                         formatter: function (value, row, index) {
		...
		$('#end_ruyuan_time').datetimepicker({
			...
			$(document).on('click', "#btn_check",function(){									
				...
				if (arrselectedData.length <= 0) {
                ...
                }else if(arrselectedData.length > 0  && row.requisition_status == 2){
                	var select_id = '';
			       ...
			       //药品校对
			        ...
			        function updateRequisition_List(select_id,time){
			        	 $.ajax({
								url: "/hospital/update_cancelRequisition_List",
								type:"post",
								data:{
									select_id : select_id,
									check_return_requisition_time: time,
									return_pharmacist : "${USER.user_work_id}",
								},
								 success: function (data) {
									 swal({
					                	   title:"药品核对成功",
                 ...
                 }else if(arrselectedData.length > 0  && row.requisition_status == 3){
                	 swal({
	                	   title:"所选的行有核对过",
	                	   text:"",
	                	   type:"warning"
			...
			$(document).on('click', "#btn_check_cancel",function(){									
				...
				if (arrselectedData.length <= 0) {
					swal({
                    	...
                    	if(row.requisition_status == 2){
                    		swal({
    		                	   title:"所选的行有未核对",
    		                	   text:"",
    		                	   type:"warning"
    		                   });
 				}else if(row.requisition_status == 4){
 					swal({
	                	   title:"所选的药品已退药,无法取消",
	                	   text:"",
	                	   type:"warning"
	                   });
 				}else if(row.requisition_status == 3){
 					var select_id = '';
                   	for (var i = 0; i < arrselectedData.length; i++) {
                   		select_id += arrselectedData[i]['requisition_list_id'] + ",";
                       }
   			       ...
   			       //取消校对
   			        ...
   			        function cancelRequisition_List(select_id){
   			        	 $.ajax({
   								url: "/hospital/cancel_cancelRequisition_List",
   								type:"post",
   								data:{
   									select_id : select_id,
   								},
			...
			$(document).on('click', "#btn_action",function(){									
				...
				if (arrselectedData.length <= 0) {
                	...
                	if(row.requisition_status != 3 && row.requisition_status != 4){
                		swal({
		                	   title:"所选的行有未校对",
		                	   text:"",
		                	   type:"warning"
			        ...
			        var patient_id = $('#patient_message_id1').val();
				       //执行医嘱
				        ...
				        function actionRequisition_List(select_id,time){
				        	 $.ajax({
									url: "/hospital/action_cancelRequisition_List",
									type:"post",
									data:{
										action_return_requisition_time : time,
										select_id : select_id,
									 ...
									 success: function (data) {
						                ...
						                $('#tb_patient_message').bootstrapTable('refresh');
						                $.ajax({
			   								url: "/hospital/chongjian_Payment",
			   								type:"post",
			   								data:{
			   									select_id : select_id,
