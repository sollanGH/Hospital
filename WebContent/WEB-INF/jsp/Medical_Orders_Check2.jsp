	 ...
	 <body>
		...
		<div class="btn-group" style="margin-left: 20px;">
	      <button id="btn_check" type="button" class="btn btn-white" >
      			<span class="glyphicon glyphicon-pencil"></span>医嘱校对
    	</button>	        
		</div>
		<div class="btn-group" style="margin-left: 20px;">
	      <button id="btn_return" type="button" class="btn btn-white" >
      			<span class="glyphicon glyphicon-pencil"></span>医嘱回退
    	</button>	        
		</div>
		<div class="btn-group" style="margin-left: 20px;">
		...
		<div class="btn-group" style="margin-left: 20px;">
	      <button id="btn_check2" type="button" class="btn btn-white" >
      			<span class="glyphicon glyphicon-pencil"></span>医嘱校对
    	</button>	        
		</div>
		<div class="btn-group" style="margin-left: 20px;">
	      <button id="btn_return2" type="button" class="btn btn-white" >
      			<span class="glyphicon glyphicon-pencil"></span>医嘱回退
    	</button>	        
		</div>
		<div class="btn-group" style="margin-left: 20px;">
		...
		oTableInit.Init = function() {
			$('#tb_patient_message').bootstrapTable({
				url: '/hospital/select_Action_Medical_Orders_List', //请求后台的URL（*）
				method: 'get', //请求方式（*）
...
var TableInit2 = function() {
		...
		oTableInit2.Init = function() {
			$('#tb_patient_message2').bootstrapTable({
				url: '/hospital/select_Action_Medical_Orders_List_Long', //请求后台的URL（*）
				method: 'get', //请求方式（*）
		...
		oInit.Init = function() {
			...
			$(document).on('click', "#btn_check",function(){									
				...
				if (arrselectedData.length <= 0) {
                ...
                }else if(arrselectedData.length > 0  && row.check_status == 0){
                	var select_id = '';
			        ...
			        function check_Medical_Orders_List(select_id,time){
			        	 $.ajax({
								...
								data:{
									...
									select_id : select_id,
									check_time: time,
									check_status : 1,patient_id : patient_id,
                 ...
                 }else if(arrselectedData.length > 0  && row.check_status == 1){
                	 swal({
	                	   title:"所选的行有校对过",
	                	   text:"",
	                	   type:"warning"
			...
			$(document).on('click', "#btn_return",function(){									
				...
				if (arrselectedData.length <= 0) {
			       ...
			       //回退医嘱
			        ...
			        function check_Medical_Orders_List(select_id,time){
			        	 $.ajax({
								url: "/hospital/return_Medical_Orders_List",
								type:"post",
								data:{
									check_nurse_id : "${USER.user_work_id}",
									select_id : select_id,
                 ...
                 }else if(arrselectedData.length > 0  && row.check_status == 2){
                	 swal({
	                	   title:"医嘱已回退",
	                	   text:"",
	                	   type:"warning"
			...
			$(document).on('click', "#btn_return2",function(){									
				var arrselectedData = $('#tb_patient_message2').bootstrapTable('getSelections');
				var row = arrselectedData[0];						
				if (arrselectedData.length <= 0) {
					swal({
			        ...
			        console.log("==========="+medical_orders_id111);
				       //执行医嘱
				        ...
				        function action_Medical_Orders_List(select_id,time,medical_orders_id111){
				        	 $.ajax({
									url: "/hospital/action_Medical_Orders_List",
									type:"post",
									data:{
										action_time : time,
										select_id : select_id,
									 ...
									 success: function (data) {
						                ...
						                function add_Requisition_List(select_id,time,requisition_id){
						                	$.ajax({
				   								url: "/hospital/add_Requisition_List",
				   								type:"post",
				   								data:{
				   									select_id : select_id,
			...
			$(document).on('click', "#btn_printf2",function(){
				...
				$.ajax({
						url: "/hospital/printf_Medical_Orders_List_Long",
						type:"get",
						data:{
							pageSize: $('#tb_patient_message').bootstrapTable('getOptions').pageSize, //页面大小
							pageNumber: $('#tb_patient_message').bootstrapTable('getOptions').pageNumber,  //页码
							patient_id: "${patient_id}",
						},
						 success: function (data) {						 
			                $('#tb_patient_message2').bootstrapTable('refresh');
			...
			$(document).on('click', "#btn_printf",function(){
				//var pageSize = $('#tb_patient_message').bootstrapTable('getOptions').pageSize;
				//console.log(pageSize);
							 $.ajax({
									url: "/hospital/printf_Medical_Orders_List_Monent",
									type:"get",
									data:{
										pageSize: $('#tb_patient_message').bootstrapTable('getOptions').pageSize, //页面大小
										pageNumber: $('#tb_patient_message').bootstrapTable('getOptions').pageNumber,  //页码
