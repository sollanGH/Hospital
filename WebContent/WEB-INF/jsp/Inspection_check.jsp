	...
	<div class="panel-body" style="padding-bottom:0px;">
		...
		<div id="toolbar" class="btn-group">					
	   		<button id="btn_check" type="button" class="btn btn-white">
              	<span class="glyphicon glyphicon-pencil" ></span>接受标本
	          	</button>
    </div>
	...
	var TableInit = function() {
		...
		oTableInit.Init = function() {
			$('#tb_patient_message').bootstrapTable({
				url: '/hospital/select_Inspection', //请求后台的URL（*）
				method: 'get', //请求方式（*）
		...
		oInit.Init = function() {
			...
			$(document).on('click', "#btn_check",function(){									
				...
				if (arrselectedData.length <= 0) {
			        ...
			        $.ajax({
						url: "/hospital/check_Inspection",
						type:"post",
						data:{
							recipients : "${USER.user_work_id}",
							select_id : select_id,
							acceptance_time: time,							
						},
						 success: function (data) {
							 $.ajax({
									url: "/hospital/add_Inspection_Payment_message",
									type:"post",
									data:{
