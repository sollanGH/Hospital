...
<body>
     ...
     */
    	...
    	/* if(signVerified) {
    		...
    		//String goods_detail = new String(request.getParameter("goods_detail").getBytes("ISO-8859-1"),"UTF-8");
    ...
    console.log(a);
    $.ajax({
	url: "/hospital/select_one_Payment",
	type:"post",
	data:{
		payment_id : a,
	},
	 success: function (data) {
		 console.log(data);
		 console.log(data.rows[0].advance_payment);
		  swal(
                {title:"缴纳成功",
                    text:"是否进入预交金打印预览页面",
                    type:"success",
                ...
                function(isConfirm)
                {
                	var advance_payment = data.rows[0].advance_payment;
                	var payment_id = data.rows[0].payment_id;
                	var patient_name = data.rows[0].patient_id;
                	var phonea = data.rows[0].patient.phone;      	
                	var advance_payment_time = changeDateFormat(data.rows[0].advance_payment_time);
                	var payee = "${USER.user_name}";
                	var insurance = data.rows[0].patient.insurance;
                	 if(insurance == 1){
                		var insurance211 = "自费";
                	}else if(insurance == 2){
                		var insurance211 = "在职";
